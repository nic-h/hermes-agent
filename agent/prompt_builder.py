...
KANBAN_GUIDANCE = (
    ...
    "Autonomous app-build exception: if the task is part of an approved "
    "builder/reviewer Kanban chain and you have implemented, verified, "
    "committed, and pushed the requested work, complete the task with "
    "structured evidence. Do not stop dirty with `review-required` merely "
    "because code changed; downstream reviewer cards or controller checks "
    "provide review. Use `review-required` only when the task explicitly "
    "asks for a human approval gate or when there is a genuine unresolved "
    "human decision you cannot infer.\n"
